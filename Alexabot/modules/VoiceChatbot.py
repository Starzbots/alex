...
from Alexabot import pbot as LYCIA
...
async def Lycia(_, message):
    ...
    try:
        L = await fetch(f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Alexa&ownername=@always_don&user=1")
        chatbot = L["message"]
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @always_don")
